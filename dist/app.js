(function(){
  ...
  const C=window.LottoCore,P=window.LottoPortfolio;
  ...
  function loadSaved(){
    try{
      ...
      if(!Array.isArray(parsed)||parsed.some(b=>!b||typeof b.id!=='string'||typeof b.label!=='string'||!Number.isFinite(Date.parse(b.createdAt))||!Array.isArray(b.games)||!b.games.length||b.games.some(n=>!C.validNumbers(n,6))))throw new Error('invalid');
      saved=parsed;
  ...
  function renderCurrent(){
    ...
    const jackpot=C.jackpotProbability(games),percent=(jackpot.probability*100).toFixed(8);
    const reciprocal=jackpot.unique?jackpot.total/jackpot.unique:null;
    const reciprocalText=reciprocal===null?'번호를 생성하면 표시됩니다.':(Number.isInteger(reciprocal)?'':'약 ')+fmt(Math.round(reciprocal))+'분의 1';
    $('jackpot-title').textContent='현재 '+m+'게임의 1등 확률';
    $('jackpot-percent').textContent=percent;
    $('jackpot-reciprocal').textContent=reciprocalText;
    $('jackpot-fraction').textContent=fmt(jackpot.unique)+' / '+fmt(jackpot.total);
    $('jackpot-coverage').textContent='전체 '+fmt(jackpot.total)+'개 조합 중 서로 다른 '+fmt(jackpot.unique)+'개를 담았습니다.';
    $('unique-badge').textContent='서로 다른 조합 '+fmt(jackpot.unique)+'개';
    $('jackpot-note').textContent='같은 수의 서로 다른 조합이면 1등 확률은 같습니다. 같은 게임 수로 번호를 다시 만들거나 보관함에 저장해도 현재 묶음의 확률은 바뀌지 않습니다.';
    $('jackpot-calculation').textContent='현재 묶음: '+fmt(jackpot.unique)+' ÷ '+fmt(jackpot.total)+' ≈ '+percent+'% ('+reciprocalText+')';
    const announcement='현재 '+m+'게임, 서로 다른 조합 '+jackpot.unique+'개. 같은 한 회차에서 1등이 나올 확률은 약 '+percent+'퍼센트, '+reciprocalText+'입니다.';
    ...
    $('probability-table').innerHTML=[6,5,4,3].map(k=>'<tr'+(k===6?' class="jackpot-table-row"':'')+'><th scope="row">'+labels[k]+'</th><td>'+((k===6?jackpot.probability:profile.probabilities[k].probability)*100).toFixed(k===6?8:6)+'%</td><td>'+((k===6?jackpot.probability:profile.probabilities[k].randomBaseline)*100).toFixed(k===6?8:6)+'%</td></tr>').join('');
  ...
  function generate(){
    ...
    try{
      const amount=Number($('amount').value),result=P.generate(templates,amount);
      current={id:globalThis.crypto?.randomUUID?.()||Date.now()+'-'+C.randomInt(1000000),createdAt:new Date().toISOString(),label:amount<=7?'번호 비중복 배치':'게임 간 겹침 최대 1개 배치',method:result.method,model:null,options:{},games:result.numbers,profile:result.profile};
      ...
      $('method-note').textContent=amount<=7?'게임끼리 같은 번호를 쓰지 않도록 구성합니다.':'45개 번호를 고르게 사용하고, 두 게임의 공통 번호는 최대 1개로 제한합니다.';
  ...
  function exportCSV(batches){
    ...
    const rows=[['생성 시각','구성 방법','게임','번호1','번호2','번호3','번호4','번호5','번호6']];
    batches.forEach(b=>b.games.forEach((numbers,i)=>rows.push([time(b.createdAt),b.label,String.fromCharCode(65+i),...numbers])));
  ...
  $('saved-list').addEventListener('click',event=>{
    ...
    if(deleteButton){const batch=saved.find(b=>b.id===deleteButton.dataset.delete);if(batch&&window.confirm('이 묶음의 '+batch.games.length+'게임을 보관함에서 삭제할까요?')&&persist(saved.filter(b=>b.id!==batch.id))){renderSaved();toast('묶음을 삭제했습니다.');}}
  });
  ...
  window.addEventListener('storage',event=>{if(event.key===storageKey||event.key===null){saved=[];storageIssue='';loadSaved();renderSaved();}});
  loadSaved();renderSaved();generate();if(storageIssue)toast(storageIssue,true);
  if(document.modelContext?.registerTool){
    const lifecycle=new AbortController();
    const actions=[{name:'read_lotto_collection',title:'현재 로또 조합 확인',description:'현재 번호와 묶음의 정확한 당첨 확률, 보관 게임 수를 읽습니다.',annotations:{readOnlyHint:true},run:()=>({current,savedGames:saved.reduce((n,b)=>n+b.games.length,0),forecastAdvantage:false})},
